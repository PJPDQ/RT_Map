L.Control.SliderControl = L.Control.extend({
    options: {
        position: 'topright',
        layers: null,
        timeAttribute: 'actual_timestamp',
        isEpoch: false,     // whether the time attribute is seconds elapsed from epoch
        startTimeIdx: 0,    // where to start looking for a timestring
        timeStrLength: 20,  // the size of  yyyy-mm-dd hh:mm:ss - if millis are present this will be larger
        maxValue: -1,
        minValue: 0,
        showAllOnStart: true,
        markers: null,
    ...
    startSlider: function () {
                    ...
                    if(_options.range){
                        ...
                        for (i = ui.values[0]; i <= ui.values[1]; i++){
                           if(_options.markers[i]) {
                               ...
                               if (_options.markers[i-1]) map.removeLayer(_options.markers[i-1]);
                           }
                        }
                        ...
                        for (i = _options.minValue; i <= _options.maxValue; i++) {
                            if(_options.markers[i].options.time == currentTime) {
                                map.addLayer(_options.markers[i]); 
                                map.removeLayer(_options.markers[i-1]);
                            }
                        }
                    }else{
        ...
        for (i = _options.minValue; i <= index_start-1; i++) {
            _options.map.addLayer(_options.markers[i]);
            // _options.map.removeLayer(_options.markers[i-1]);
        }
    }
});
...
//         // Create a control sliderContainer with a jquery ui slider
//         var sliderContainer = L.DomUtil.create('div', 'slider', this._container);
//         console.log("slider = ", sliderContainer);
//         $(sliderContainer).append('<div id="leaflet-slider" style="width:200px"><div class="ui-slider-handle"></div><div id="slider-timestamp" style="width:200px; margin-top:13px; background-color:#FFFFFF; text-align:center; border-radius:5px;"></div></div>');
//         //Prevent map panning/zooming while using the slider
//         console.log("mouse down = ", sliderContainer);
//         $(sliderContainer).mousedown(function () {
//             map.dragging.disable();
//         });
//         // console.log("document about to mouse up... = ", sliderContainer);
//         $(document).mouseup(function () {
//             map.dragging.enable();
//             //Hide the slider timestamp if not range and option alwaysShowDate is set on false
//             if (options.range || !options.alwaysShowDate) {
...
//                             currentTime = _options.markers[ui.value].options.time;
//                         }
//                         console.log("is samedate? = ", _options.sameDate);
//                         console.log("curr time = ", currentTime);
//                         console.log("maxVal = ", _options.maxValue);
//                         console.log("maxVal = ", _options.minValue);
//                         for (i = _options.minValue; i <= _options.maxValue; i++) {
//                             if(_options.markers[i].options.time == currentTime) map.addLayer(_options.markers[i]);
//                         }
//                     }else{
